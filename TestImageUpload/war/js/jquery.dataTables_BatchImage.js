			...
			"fnInit": function ( oSettings, nPaging, fnCallbackDraw )
			{
				...
				$(nNext).bind( 'click.DT', function() {
					if ( oSettings.oApi._fnPageChange( oSettings, "next" ) )
					{
						...
						$('#message').html("<B>Loading Schedule BatchMessage List</B>");
						$('#message').show();
						$('#message').fadeOut(5000); 
						//alert(document.getElementById("tab").getElementsByTagName("tr").length)
						if(document.getElementById("ImageTable").getElementsByTagName("tr").length < 5){

							$('#message').html("<B>End of Record</B>");
							$('#message').show();
							$('#message').fadeOut(7000); 
							return;
						}
						
						$.ajax({type: 'GET', url: '/api/ImageUpload/getImageWithRequestPagination/v_0' , success: function(data){
							//alert(data)
							
							
							if(data!=null){
								var tempSchedulEventList=data;
								//alert("The data for schedule batch massage "+tempSchedulEventList)
								batchImageList = copyArray(tempSchedulEventList, batchImageList);
								structuringScheduleBatchMessage(tempSchedulEventList);
							}
						}
						});
						function structuringScheduleBatchMessage(tempbatchImage){
							var newRow = new Array();
							temp = Jdatatable.fnSettings().aoData.length;
							//alert(temp)
							var dataIndex = temp; 
							$.each(tempbatchImage, function(index, tempbatchImage) {
								 newRow[0]="<input name='deleteBatchCheckBox' type='checkbox' id='" + tempbatchImage.imageId + "' value='Deleterow'>";
								  
								   var imageName = tempbatchImage.title;
								   newRow[1] =imageName;
										
								   var imageType1= tempbatchImage.imageType;
								   newRow[2]=imageType1;
								   
								   var batchStatus = tempbatchImage.status;
								   newRow[3]=batchStatus;			
								  
								   newRow[4]="<input name='showImage' type='button' id='" + tempbatchImage.imageId + "' value='ShowImage' onclick='showImageonclick(\""+tempbatchImage.imageId+"\", " + dataIndex + ")'>";
							...
							Jdatatable.fnSettings()._iDisplayStart = temp - 4;
							if ( oSettings.oApi._fnPageChange( oSettings, "next" ) ){
								fnCallbackDraw( oSettings );
							}
						}
