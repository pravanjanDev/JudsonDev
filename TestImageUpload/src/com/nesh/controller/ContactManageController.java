...
public class ContactManageController {
		...
		String responseMessage = "";
		 ...
		 try{
			...
			if((Boolean) resultMap.get("flag")){
				response.sendRedirect("/jsp/landing-page.jsp");
			}
			else{
	...
	@RequestMapping("/getAllcontact/v_0")
	public @ResponseBody HashMap<String, Object> getAllcontact(HttpServletRequest request ,HttpServletResponse response){
		
		HashMap<String,Object > resultMap = new HashMap<String,Object>();

		 try{
			 resultMap =  contactHelper.getAllcontact(request,response);
		 } catch (Exception e) {
			 StringWriter sw = new StringWriter();
			 PrintWriter pw = new PrintWriter(sw);
			 e.printStackTrace(pw);
