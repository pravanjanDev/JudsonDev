...
public class ImageUploadController {
	...
	@RequestMapping("/getAllImage/v_0")
	public HashMap<String, Object> getAllImage(HttpServletRequest request ,HttpServletResponse response){
		ImageHelper imageHelper = new ImageHelper();
		HashMap<String,Object> resultMap = new HashMap<String, Object>();
		 try{
			 resultMap = imageHelper.getApproveImageWithRequestPagination(request, response);
	...
	@RequestMapping("/getApproveImageWithRequestPagination/v_1")
	public @ResponseBody LinkedHashMap<String, Object> getApproveImageWithRequestSortingOrder(HttpServletRequest request ,HttpServletResponse response){
		ImageHelper imageHelper = new ImageHelper();
		LinkedHashMap<String,Object> resultMap = new LinkedHashMap<String, Object>();
		 try{
			 resultMap = (LinkedHashMap<String, Object>) imageHelper.getApproveImageWithRequestSortingOrder(request, response);
			 System.out.println("the response "+resultMap);
		 } catch (Exception e) {
			 StringWriter sw = new StringWriter();
			 PrintWriter pw = new PrintWriter(sw);
			 e.printStackTrace(pw);
	...
	@RequestMapping("/getRejectedImageWithRequestPagination/v_0")
	public HashMap<String, Object> getRejectedImageWithRequestPagination(HttpServletRequest request ,HttpServletResponse response){
		ImageHelper imageHelper = new ImageHelper();
		HashMap<String,Object> resultMap = new HashMap<String, Object>();
		 try{
			 resultMap = imageHelper.getRejectedImageWithRequestPagination(request, response);
		 } catch (Exception e) {
			 StringWriter sw = new StringWriter();
			 PrintWriter pw = new PrintWriter(sw);
			 e.printStackTrace(pw);
    ...
    @RequestMapping(value ="/webUpload/v_1", method = RequestMethod.POST)
    public   void webUpload(CommentForm commentForm,HttpServletRequest request , HttpServletResponse response) {
    	
    	HashMap<String,Object> paramMap = new HashMap<String, Object>();
    	String responseMessage ="";
    	try{
    		paramMap = imageHelper.uploadImageByFile(commentForm);
    		responseMessage = "Your Image has been uploaded sucessfully";
    		response.sendRedirect("/jsp/image-upload.jsp?responseMessage="+responseMessage);
    	}
    	catch(Exception e){
    		 StringWriter sw = new StringWriter();
			 PrintWriter pw = new PrintWriter(sw);
